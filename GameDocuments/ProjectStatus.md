...
- 2026-09-26: 걷기 대체 연출(좌우 기울기·상하 튐)을 끔(`walkTiltDegrees`·`walkBobHeight` 0, MCP). 실제 걷기 애니메이션 도입 예정이며 코드는 유지해 값만 올리면 다시 켤 수 있음.

- 2026-09-26: 분대 결속 이동 2단계. 전진·집중·분산 명령에서 분대가 블록 모양을 유지하며 이동함. 블록 첫 자리 인물(보통 분대장)이 선두로 가장 가까운 적(집중이면 표적) 쪽 접촉 지점을 향하고, 나머지는 선두 기준 블록 자리를 따라감. 분대원이 평균 1.2 이상 뒤처지면 최대 1.5초 기다림. 근접 위주 분대는 근접 인물이, 원거리 위주 분대는 원거리 인물이 사거리 안에서 적과 맞닿으면 전진을 멈추고, 이후 분대원은 자리에서 최대 1.2 안에서만 교전(원거리는 재장전 중 분대와 함께 이동). 앞줄이 쓰러지면 살아 있는 인원으로 자리 순번을 다시 매겨 뒤 인물이 앞을 채움. 돌격 위주 분대는 적과 4 안에서 속도 ×1.35로 돌진. 사수·이동·집결·직접 지휘·후퇴·붕괴는 기존 동작 유지. 구현 중 두 가지 교착(원거리 인물의 사거리 여유 판정, 혼합 분대의 원거리 인물이 근접 분대 전진을 멈춤)을 발견해 수정. 첫 공성 전투(카르디아) 54.8초 승리로 종료. 전투·영웅 스킬 EditMode 44/44, 증원·접경 방어·공동 출정·카르디아 진단 25/25 통과. 60명 600스텝 225ms. 30대30 화면에서 블록 행군과 블록 충돌 확인(증거: DesignAuditEvidence/Battle-Cohesion-March.png, Battle-Cohesion-Clash.png). 여러 분대가 같은 적 분대로 몰리는 현상은 남아 있음(분대 단위 표적 배분은 후속).

- 2026-09-26: 직업별 전투 병과 7종 추가(`WI_BattleConfig.classProfiles`, 직업 12개 매핑·체력/피해/이동/공격 간격/사거리 배율). 방진(수호자): 멈춰 서면 정면 근접 피해 ×0.6·돌격 무력화·밀림 저항, 측면·후방 피해 ×1.25 추가. 돌격(검성·마검사): 2.5 이상 달려온 첫 타격이 피해 ×2·밀어냄 ×3·적 분대 사기 -4·화면 흔들림. 유격(암살자): 이동 ×1.3, 후방 ×2, 원거리·지원 표적 선호. 궁병(궁수): 같은 분대 준비된 궁수가 함께 쏘는 일제 사격, 포물선 화살이 목표 지점에 떨어져 반경 0.8 피해. 술사(대마법사·흑마법사·연금술사): 공격 간격 ×1.3~1.5·피해 ×1.2~1.5, 명중 지점 반경 1 주변 피해(50%). 지원(사제·드루이드): 사거리 안 체력 75% 미만 아군 중 가장 다친 인물 치료(12). 지휘(성전사·책사): 반경 4 아군 분대 사기 회복 +2/초·사기 손실 ×0.7. 인물 정보에 병과와 특징 표시(UID 7종). 30대30 검증을 진영별 5명 전투단 6개(지휘·방진, 돌격, 궁병, 술사, 유격, 지휘·궁병)의 병과 혼합 편성으로 변경. 전투·영웅 스킬 EditMode 43/43, 증원·접경 방어·공동 출정 22/22 통과. 60명 600스텝 221ms. 30대30 교전 캡처에서 포물선 화살·마법탄·근접 혼전 확인(증거: DesignAuditEvidence/Battle-MixedClasses-30v30.png). 병과 간 밸런스는 실플레이 조정 필요.

- 2026-09-26: 전투 게임필(손맛) 연출 추가. 전투 결과에는 영향 없음. (1) 캐릭터 프리팹 구조 변경(MCP): 인물 그림을 루트에서 `Body` 자식 SpriteRenderer로 분리해 발밑 링·체력 바는 고정한 채 몸만 움직임. (2) 캐릭터: 걷기 상하 튐·기울기, 근접 공격 내딛기, 피격 붉은 번쩍임·눌림, 전투 불능 시 넘어지며 사라짐(이탈은 흐려짐), 증원 등장 튀어나옴, 선택 분대 표시 맥동. (3) 카메라: 트라우마 방식 화면 흔들림(첫 충돌·분대장 격파·스킬 명중·분대 붕괴·격파·측면 타격), 기준 위치와 흔들림 분리. (4) 히트 스톱: 첫 충돌 0.08초·분대장 격파 0.14초·스킬 명중 0.06초 동안 전투와 연출 정지. (5) 이동·공격·배치 명령 지점에 퍼지는 원(이동 청록, 공격 빨강). 시뮬레이션은 `FeedbackEvents`에 사건만 기록하고 표시 계층이 프레임마다 소비. 모든 수치는 `WI_BattleConfig` Game Feel 항목. 전투·영웅 스킬 EditMode 40/40, 증원·접경 방어·공동 출정 22/22 통과. 30대30 교전 캡처에서 피격 번쩍임 확인, 움직임 연출·흔들림·히트 스톱은 정지 캡처로 확인 불가하여 실플레이 확인 필요. 효과음 에셋이 프로젝트에 없어 소리 연출은 미포함.

- 2026-09-26: 분대 블록 진형 1단계. 분대 편성을 "영웅마다 1분대"에서 "전투단 1개 = 분대 1개"로 변경(실제 전투단 권장 인원은 대장 통솔 기준 4~8명, `squadMaxSize` 8 초과 시 분할, 분대장은 대장>영웅>첫 인물). 초기 진형을 진영 전체 역할 열 대신 분대 블록으로 재작성: 분대 안은 3열(`squadBlockFiles`) 사각 블록, 분대장이 앞줄 중앙, 전위→근접→원거리 순. 근접 위주 분대는 전열, 원거리 위주 분대는 후열(`formationLineGap` 1.5), 전열 안에서는 중앙부터 위아래 번갈아 배치(`squadGap` 1.2), 넘치면 뒤 줄. 증원 분대는 진영 가장자리 쪽 블록. 자리 흔들림(`formationJitter` 0.12)과 개인 이동 속도 편차(`moveSpeedVariance` 6%)를 인물 ID 기반 결정적 값으로 추가. 기존 `formationColumnSpacing`·`formationRowSpacing` 제거. 전투 테스트 랩은 실제와 같이 5명씩 전투단으로 나눔. 전투·영웅 스킬 EditMode 40/40, 증원·접경 방어·공동 출정 22/22 통과. 30대30 배치 화면에서 블록 진형 확인(증거: DesignAuditEvidence/Battle-SquadBlocks-30v30.png). 전투 개시 후 분대원은 아직 각자 표적을 쫓으므로 블록이 유지되지 않음(2단계 분대 결속 이동에서 처리 예정).

- 2026-09-26: 전투 맵·카메라 확장. 기존 배경(36×20)에서 싸울 수 있는 흙바닥이 약 24×13이라 논리 전장만 넓히면 성벽·천막 위로 이동하게 되므로, `arenaPrefabScale`(1.35)을 추가해 전장 프리팹을 키우고 논리 전장 `arenaSize`를 18×10→32×18, 카메라 경계 `arenaBackgroundSize`를 48.6×27.34로 변경(캐릭터 크기 유지). 시작 전위 거리 6→11(두 진영 간격 약 22), 배치 구역 비율 0.4→0.3(가장자리에서 9.6), 지형 3구역을 새 크기에 맞게 이동·확대. 카메라 줌 A/B/C 6/9/13.5(C는 전장 전체), 이동 속도 10, 확대 배율에 비례한 키 이동, 마우스 휠 줌을 커서 위치 기준으로, 가운데 버튼 드래그 이동 추가. 모두 MCP로 데이터 적용. 전투·영웅 스킬 EditMode 40/40, 증원·접경 방어·공동 출정 22/22 통과. 30대30 배치 화면 확인(증거: DesignAuditEvidence/Battle-Arena-32x18.png). 전장 우상단 성벽·좌측 울타리가 논리 전장 모서리와 약간 겹칠 수 있음. 전체 보기에서 캐릭터가 작아짐. 휠 줌·가운데 드래그 실조작은 미검증.

- 2026-09-26: 전략 지도·성 내정 화면을 UIConcepts 시안에 다시 맞춤(MCP). 원인 조사에서 확인한 항목만 수정함. 성 화면: 가로형 수치판 `castle_stat_*`가 아이콘 칸에 들어가 베이지 막대로 보이던 9곳을 새 소형 투명 아이콘 `icon_castle_stat_*_v1`(수치판에서 아이콘만 잘라 냄) 및 기존 `icon_flat_military/heroes`로 교체함. 09-25 인재 활동 버튼 제거로 생긴 명령 메뉴 빈칸을 없애고 6개 명령을 균등 재배치함. 특화 시설은 두 칸을 항상 표시하고 잠긴 칸은 `UI_ADMIN_FACILITY_SLOT_LOCKED`, 빈 칸은 `UI_ADMIN_FACILITY_SLOT_EMPTY` 문구로 표시함(`WIAdministrationSlotSnapshot.Unlocked` 추가). 전략 화면: 08-23 기록 없이 범용 `bg_type_a/f`로 바뀐 좌측 성 패널·영웅 칸·우측 이번 달 패널·목표 카드·전투 알림을 08-22의 시안 전용 아트(`right_panel_frame`, `strategy_hero_card_v1`, `bg_type_d`, `right_objective_card`, `right_danger_row`)로 되돌림. 지도 성 마커 40→56, 선택 확대 1.65→1.35배, 이름표 12/15→14/17pt. 두 화면 주요 글자를 1920 기준 12~17pt에서 14~32pt로 올림. 성 규모 enum이 `Small`처럼 그대로 보이던 문제를 `GetCastleSizeName`과 `UI_CASTLE_SIZE_*` UID로 수정함. 09-25 정리 때 삭제된 exec 이미지 2장은 실제 참조 시안이었으므로 `Objective_UI_Concept_V1.png`, `TurnFollowup_UI_Concept_V1.png`로 복원하고, DataDocument·DataManual의 삭제된 `UIHandoffReport.md`·옛 V1 파일명 참조를 현재 시안으로 교체함. Unity 컴파일 오류 0건, UGUI·레이아웃 관련 EditMode 104/104 통과. 전체 EditMode 473개 중 460개 통과, 실패 13개(장기 밸런스 5, 자동 플레이 재미 4, 장기 성능 1, 인물 선택 행 초상·문구 3)는 09-24 기록의 기존 실패와 같은 항목임.

- 2026-09-25: 주인공 표시명을 아레스(Ares)에서 키리엔(Kyrien)으로 변경함. 림가르드 인물은 고대 북방어 뿌리로 짓는 작명 규칙을 따르며 이름의 뜻과 결정 기록은 `NarrativeSetting.md`에 정리함. 마스터 데이터의 인물 이름, 시나리오 부제·줄거리·주인공 문구, 관계 시작 문구, 캠페인 변형 이름(`키리엔 메인`)과 설명을 MCP로 변경하고 에디터 도구 기본 문구·코드 주석·테스트 보고 문구와 현행 문서를 함께 갱신함. 내부 ID(`ares`, `AresMain`, 문자열 UID의 `ARES`)와 이미지 경로(`Art/Characters/Ares/`)는 저장·테스트 호환을 위해 유지함. 과거 변경 기록의 아레스 표기는 당시 기록으로 남김. Unity 컴파일 오류 0건, 시나리오 선택·UGUI EditMode 53/53 통과.

- 2026-09-25: 배치 구역 표시를 타원에서 사각형으로 변경함. 흰 사각형 Sprite `WI_ZoneRect_V1.png`와 `WIBattleZoneRectMarker.prefab`을 제작해 BattleScene `BattleRuntime.rectZoneMarkerPrefab`에 연결(MCP). 초기 진형 간격을 넓힘: `formationFrontLineDistance`(6, 중앙~전위 열 거리)를 추가해 두 진영 전위 사이가 약 12가 되도록 하고 가장자리 안으로 제한. 기존 진형 계산이 방향을 뒤집어 원거리 열이 전위보다 중앙 쪽에 서던 버그도 수정(이제 전위가 앞, 원거리가 뒤). 전투·영웅 스킬 EditMode 40/40, 증원·접경 방어·공동 출정 22/22 통과. 30대30 배치 화면에서 사각형 구역과 벌어진 시작 위치, HUD 표시 확인(증거: DesignAuditEvidence/Battle-Deployment-RectZone-30v30.png).

- 2026-09-25: `GameDesign.md` 15장 실시간 전투를 현재 구현 기준으로 재구성함. 15.1 방향(분대 RTS + 영웅 직접 지휘), 15.2 분대, 15.3 조작 키표, 15.4 배치 단계, 15.5 근접(교전 슬롯·측면/후방), 15.6 원거리, 15.7 영웅 스킬(위치 지정), 15.8 명령, 15.9 사기와 붕괴, 15.10 지형, 15.11 전투 결과. 미구현 항목(공격 동작·경직, 투사체 빗나감·차단, 돌진·보호막 등 스킬 확장, 분대 직접 편성)은 후속으로 표기함. 이전 기록의 15.6 등 절 번호는 개정 전 기준.
...
## 임시 전투 이펙트 게임 연결 완료 (2026-09-25)
- 앞선 프리팹 단독 제작 기록 이후 게임 연결을 완료했다. BattleConfig에 검격/화살/마법/피격 프리팹 4종을 연결하고 WIBattleEffectPresenter가 사전 제작 프리팹을 복제·재사용한다.
- 근접 공격은 검격, Ranged 역할은 화살, Magic/Support 역할은 마법탄을 사용한다. 근접·투사체 충돌·공격 스킬 피해 발생 시 피격 이벤트를 표시한다. 투사체 자체 속도는 인스턴스에서 끄고 실제 전투 좌표·방향에 맞추며 명중/만료까지 외형을 유지한다.
- 단발 효과는 한 번만 재생하고 완료 후 풀에 반환한다. 씬 종료 시 먼저 파괴된 자식 참조를 건너뛰도록 정리 로직을 보강했다. 기존 배치 단계·전술 일시정지·스킬 범위 표시는 유지한다.
- 실제 30대30 테스트 전투 컨트롤러를 1/30초씩 진행해 2.75초 마법, 2.80초 화살, 3.40초 검격/피격 표시를 확인했다. 증거는 DesignAuditEvidence/BattleEffects-{Slash,Arrow,Magic,Hit}.png와 BattleEffects-Integration.txt. UI 클릭 검증은 제외했다. 플레이 검증 및 종료 후 콘솔 오류 0건.
- 최신 배치/전술 코드와 함께 이펙트 신규 6개 및 기존 전투/투사체/성능 회귀 7개, 총 13/13 통과(job 4d412a6b7ff044cd9e31bc1399acb353). 플레이 모드 종료 및 컴파일 오류 없음. 커밋·푸시 없음.
