...
- 2026-08-29: 월드와 성 내정 화면이 서로 다른 다음 턴 버튼을 보유하던 구성을 `WIAdministrationEndTurnUGUI.prefab` 공용 중첩 프리팹으로 통합함. 공용 컨트롤러가 다음 턴 클릭 연결, 미결 전투에 따른 활성 상태와 문구를 함께 관리하며 두 화면은 동일한 버튼 그래픽·문장·화살표·크기 구성을 사용함. 중첩 프리팹 회귀 검사 통과, 실제 Avalon Castle QA와 Console Error 0건 확인.
...
- 2026-08-28: 강화된 AI 공격 전력 판단 때문에 전투 결과 테스트가 AI의 자율 공격 선택에 종속되던 문제를 정리함. 공통 픽스처가 발도르 전투단의 아발론 원정을 2개월 일정으로 명시 예약하도록 변경하여 전략 승패·실시간 대기/제출·난이도별 인물 운명·후퇴 결과가 AI 의사결정과 독립적으로 검증되도록 함. 승리 픽스처는 공격 증원 인물을 성 주둔 목록에서 제거하고 목표 성 방어 조건을 고정함. `WIAdministrationAITurnTests`는 169/190에서 179/191로 개선되었으며, 남은 12건은 전투 물리 6건·36개월 AI 3건·AI 공세/경제 3건임.
