...
### Data
- 던전 이벤트, 몬스터, 직업, 던전 데이터, 스킬 데이터 구조(ScriptableObject) 정의 완료 및 `Assets/Data/ScriptableObject` 하위 폴더 정책 확립
- `WI Tools -> 통합 데이터 임포터`: CSV 형태의 데이터를 유니티 ScriptableObject 에셋으로 자동 변환, 내보내기(Export) 기능 구현 및 기본 경로(`Assets/Data/Excel`) 연동 완료
- `WI Tools -> 통합 데이터 관리자`: 에디터 환경 내에서 엑셀과 유사한 형태(Grid)로 던전/몬스터/직업/스킬 데이터를 직접 열람 및 수정 가능한 실시간 툴 구축 완료
- 하드코딩되던 임시 배틀 생성 로직을 제거하고, 자동 생성된 SO 데이터를 동적으로 로드해 배틀 세션을 구성하도록 `WIBattleManager` 수정 및 연동 완료
