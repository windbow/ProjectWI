...
- 성 노드는 사각 텍스트 박스가 아니라 성채와 세력 깃발 이미지로 보여야 합니다.

### 성 내정 화면

![성 내정 목표 시안](UIConcepts/ProjectWI_Castle_Administration_UI_Concept_V1.png)

- 경로: `GameDocuments/UIConcepts/ProjectWI_Castle_Administration_UI_Concept_V1.png`
- 핵심 구조: 공통 상단 HUD / 좌측 성·태수·수치 / 중앙 성 전경 / 우측 성 명령 / 하단 영웅·시설·이번 달 중점 / 양쪽 하단 이동 버튼
...
| 성 내정 구조 | 좌측 현황, 중앙 전경, 우측 명령, 하단 슬롯 존재 | 시안의 중앙 전경 면적과 하단 카드 밀도에 맞춰야 함 |
...
주의: 최신 `Assets/Screenshots/UI_Current_Preview.png`와 `UI_Castle_Administration_Preview.png`는 현재 둘 다 캠페인 시작 레이어가 열린 상태를 캡처해 전략 지도·성 화면의 최종 비교 자료로 사용할 수 없습니다. 후속 세션은 새 캠페인 진입 후 전략 지도와 성 내정 화면을 각각 다시 캡처해야 합니다.
...
- `Assets/Resources/UI/Generated/map_castle_*.png`: 세력별 성채·깃발
...
- 아발론 소유 성을 열고 1920×1080 성 내정 캡처
...
- 문장, 세력명, 날짜, 금화, 마나, 영향력의 폭과 간격을 시안에 맞춤
...
- 좌측에 성명·태수·4대 수치·수입·진행 사업을 세로 계층화
- 우측 성 내정 명령을 동일 높이 버튼으로 정렬
- `진격 / 출정`만 위험색 사용
...
기능을 추가하는 작업이 아니라, 이미 연결된 전략 지도와 성 중심 내정 흐름을 두 시안의 시각적 완성도에 가깝게 정돈하는 작업입니다. 우선순위는 `정보 구조 일치 → 화면 비율 일치 → 타이포그래피 → 이미지·상태 디테일` 순서입니다. 사람의 미적 판단이 필요한 마지막 미세 간격과 색감은 비교 캡처를 보면서 사용자와 함께 조정합니다.
