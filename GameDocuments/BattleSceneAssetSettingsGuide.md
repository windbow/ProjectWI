...
- 키리엔 기준 전투 Sprite는 `Assets/Art/Characters/Ares/Ares_Battle_1WU_A_OutlineBake_V1.png`이며 190×256, 256 PPU, Scale 1에서 외곽선 포함 가시 높이 정확히 1월드 유닛입니다.
...
- 현재 키리엔의 약 1유닛 높이를 전투 캐릭터 표시 기준으로 사용합니다.
...
이 절차는 키리엔 검증값을 기준으로 한 1차 표준입니다. 캐릭터 아트 방향이 확정되면 목표 픽셀 높이와 Mipmap 정책은 변경할 수 있지만, 모든 캐릭터에 동일한 계산을 적용해야 합니다.
...
3. 경계 상자에는 머리카락, 망토, 무기를 포함하되 완전히 투명한 바깥 여백은 제거합니다. 키리엔은 `(x 75~939, y 116~1291)`의 865×1176px 영역입니다.
4. 잘라낸 이미지는 종횡비를 유지하며 알파 실루엣 높이를 정확히 256px로 축소합니다. 키리엔 결과는 `865×1176 → 188×256`입니다.
...
환경물 Sprite의 투명 캔버스 전체 크기와 실제 물체가 차지하는 크기는 다를 수 있습니다. 수치만 보지 말고 키리엔 1유닛 실루엣을 바로 옆에 배치해 육안으로 검증합니다.
...
2. 키리엔 또는 2유닛 높이의 기준 캐릭터를 배치해 축척 가이드를 만듭니다.
