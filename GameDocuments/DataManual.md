...
## 인물 배정 스크롤 목록 (2026-09-20)

- 영지관·사업·연구·첩보 담당자, 인물 배치·활동, 군사·원정의 8개 선택 화면은 가로 행을 세로로 스크롤합니다. 초기 행 8개는 재사용 풀의 시작 수이며 표시 가능한 전체 후보 수의 제한이 아닙니다.
- 검색은 이름과 요약 정보에 적용합니다. 전체/선택 가능 필터와 기본순/이름순 정렬을 지원합니다. 인재 활동은 기존 검색·영웅/일반 필터·추천순/이름순 도구막대를 그대로 사용합니다.
- 행은 작은 초상, 이름, 화면별 능력과 성과, 등급·직업·피로·내정 및 전투단 정보를 표시합니다. 군사 후보는 기존 소속 또는 임무로 편성 불가능한 이유를 표시합니다. 선택 상태는 검색·정렬 후에도 유지되며 하단 버튼으로 한 번에 편성합니다.
- WICharacterSelectionRow.prefab이 공통 행 원본입니다. WI/UI/Build Character Selection Lists 메뉴가 8개 화면에 스크롤과 초기 행 참조를 저장합니다. 실행 중에는 저장된 행 프리팹만 복제·재사용합니다. 영웅 도감은 기존 카드형을 유지합니다.
- 공통 문구는 UI_SELECTION_* 및 UI_ARMY_LIST_STATUS, UI_MARCH_LIST_STATUS UID로 관리합니다.
...
## 빠른 내정·점령 진행 (2026-09-19)

- 성의 기존 영지관/위임 화면에서 방침과 기본·집중 예산을 선택한 뒤 `기본 운영 시작`을 누릅니다. 영지관이 없어도 운영할 수 있으며 `기본 운영 중지`로 자동 지출을 멈춥니다. 시작 성과 기존 저장의 운영 설정은 자동으로 덮어쓰지 않습니다.
- 업무 가능한 영지관은 추가 성과를 제공하지만 출전·해임·다른 임무 때문에 기본 내정이 멈추지 않습니다. 기본 성과와 비용은 기존 `projectBalance`의 `baseGain`, `minimumGain`, `basicCost`, `intensiveCost` 등을 사용합니다. 영웅의 흔적·시설·성 특산·연구·진영 방침 보너스도 유지됩니다.
- 새 점령지는 영지관 임명 없이 `전선` 방침·기본 예산으로 자동 운영합니다. 점령 통치 선택 후 인원을 남길 의무는 없으며, 빈 성은 적에게 무혈 점령될 수 있습니다.
- `WI_AdministrationDatabase.asset > automation`의 `occupationUnrestMonths=3`은 점령 직후 불안 기간, `occupationStabilityGain=5`는 월간 질서 회복량, `victoryReorganizationMonths=0`은 승리 후 의무 재편 기간입니다. 통치 선택의 `UnrestMonths`가 초기 불안 기간을 덮어씁니다. 점령이 턴 계산 중 발생하면 그 턴의 월간 안정화도 적용됩니다.
- 승리한 전투단은 주 공격군·지원군 모두 편성을 유지해 바로 다음 이동을 명령할 수 있습니다. 피로·부상·보급·경로·교전 및 영향력 조건은 그대로 확인합니다. 패배 후 재편은 기존 1개월입니다.
- 관련 표시 문구는 `UI_ADMIN_BASIC_OPERATION`, `UI_ADMIN_OPERATION_*`, `UI_ADMIN_GOVERNOR_OPTIONAL`, `UI_ADMIN_OCCUPATION_PROGRESS` UID로 편집합니다. 기존 프리팹의 안내·버튼·보고 텍스트를 바인딩하며 새 UI 구조를 생성하지 않습니다.

## UI 프리팹 제작과 런타임 표시

- UI 계층, 레이아웃, 컴포넌트와 기본 스타일은 씬 또는 프리팹으로 미리 제작하며 실행 전부터 Unity Editor에서 확인할 수 있어야 합니다.
- 런타임 코드에서 `new GameObject`, `AddComponent`로 Canvas, Panel, Button, Image, Text, LayoutGroup 등의 UI 구조나 대체 화면을 즉석에서 만들지 않습니다.
- 완성된 화면 프리팹과 목록·카드·슬롯 항목 프리팹을 런타임에 생성, 캐시, 재사용 또는 풀링하는 것은 허용합니다. 런타임 코드는 텍스트·이미지·진행도·선택 상태·활성 상태 등의 데이터 바인딩을 담당합니다.
- 프리팹이나 직렬화 참조가 누락되면 코드로 임시 UI를 생성하지 말고 Console 오류를 확인한 뒤 에디터에서 원본 에셋과 참조를 수정합니다.
- `WIUIScreenManager`는 MainScene에 등록된 완성 화면 프리팹을 한 번씩 생성해 캐시하며, 열린 모달의 실제 순서를 중앙 스택에서 관리합니다. 개별 모달 프리팹의 고정 Canvas 순서가 같아도 나중에 연 화면이 위에 표시되고 먼저 닫힙니다.
- 생성된 화면은 대표 컨트롤러 타입으로도 등록됩니다. 화면 접근 코드는 배열 순서나 오브젝트 이름을 찾지 않고 `WIUIScreenManager.TryGetScreen<TController>`를 사용하며, 같은 컨트롤러 타입의 화면을 둘 이상 등록하지 않습니다.
- 타이틀·월드·영지 화면은 시작 시 생성됩니다. 군사·영웅·외교·계략·연구·보고·선택 화면 등의 모달은 최초 표시 요청에서만 생성되며 이후 같은 인스턴스를 재사용합니다. 새 화면 요청은 이벤트를 직접 호출하지 않고 `RaiseUGUIScreenRequest<TController>`를 통해 프리팹 준비 후 전달해야 합니다.
- MainScene의 `UGUI Screen Bootstrap > WIUIScreenManager > Administration Controller`에는 씬에 배치된 `WIAdministrationUIController`를 반드시 연결합니다. 개별 화면은 씬 검색으로 컨트롤러를 찾지 않으며 중앙 관리자가 생성 시 같은 참조를 주입합니다.
...
- 새 점령지는 인원 도착을 기다리지 않고 `전선` 방침 기본 운영을 시작합니다. 구 저장에 남아 있는 영지관 임명 대기는 다음 턴에 기본 운영으로 전환하고, 이미 대기 중인 적합한 인물이 있을 때만 선택적으로 임명합니다.
...
`Heroes` 목록은 고유 영웅뿐 아니라 `Grade`가 `Common`인 일반 인물도 함께 관리합니다. 현재 일반 인물은 1,000명이며 전투단 편성·성 방어와 자동 훈련에 참여합니다. 내정·연구·탐색·영입·첩보는 승격 후 해당 특성이 있을 때 담당합니다.
...
- 영지관 위임 UGUI에서는 주둔 인물을 영지관으로 임명하고 균형·번영·연구·전선·인재 방침과 기본·집중 월 예산을 선택합니다. 예상 자동 사업은 기존 계산 결과로 표시되며 영지관 없이도 기본 운영할 수 있고 출전 중인 영지관도 보너스를 제공합니다.
