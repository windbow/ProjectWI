...
이 문서는 `ProjectWI`의 성 이름, 성별 개성, 영지 관리 화면, 메뉴 및 명령 명칭을 정의한다. 게임 규칙은 항상 `GameDesign.md`를 우선하고 플레이어 표시 용어는 `FantasyTerminologyGuide.md`를 따른다. 이 문서의 지역 분류는 데이터 수치가 아닌 작명과 배경 설정용 태그다.

`영지 관리`의 영문 표기는 **Administration**을 사용한다. 게임 안에서 플레이어에게 보여 주는 영문 메뉴는 딱딱한 행정 업무보다 영지를 돌본다는 느낌을 주기 위해 **Realm Management**를 사용한다.
...
- 이름만 보고 어느 진영의 성인지 어느 정도 추측할 수 있어야 한다.
...
- 림가르드: 서리, 성벽과 울타리, 별과 얼어붙은 호수. 겉으로는 잃어버린 정통 왕국, 실체는 얼음 감옥 (`NarrativeSetting.md`의 림가르드 항목 참고)
- 발도르: 라틴·게르만풍 제국, 황금, 태양, 사자와 군단
- 아이언하트: 드워프·수인 연맹, 바위, 철, 서리와 야수
- 실반로드: 엘프·드루이드 의회, 숲, 달, 바람과 노래
- 네크로폴리스: 언데드·다크엘프 진영, 밤, 재, 뼈와 심연
...
|---|---|---|---|
| `Capital` | 수도 | Capital | 진영의 중심. 의회와 고급 영웅 관리 기능 제공 |
| `City` | 도시 | City | 번영과 금화 수입에 유리 |
| `Fortress` | 요새 | Fortress | 방어와 전선 유지에 유리 |
| `Arcane` | 마도성 | Arcane Hold | 기술과 마나 크리스탈에 유리 |
| `Sanctuary` | 성역 | Sanctuary | 질서, 회복과 인재 교류에 유리 |

## 4. 60개 성 목록

성 ID는 현재 데이터의 `castle_00`~`castle_59`를 유지한다. 진영별 시작 성 개수도 기존 기획과 동일하게 림가르드 1, 발도르 25, 아이언하트 12, 실반로드 12, 네크로폴리스 10으로 구성한다.

### 4.1 림가르드 백작국 — 1개

| ID | 한글명 | 영문명 | 지역 인상 | 특산·랜드마크 | 영지 관리 개성 |
|---|---|---|---|---|---|
| `castle_00` | 림가르드 | Rimgard | 수도 | 별빛 호수 | 모든 사업의 최소 성공률이 보장되며 영웅 방문 사건이 자주 발생 |

림가르드는 시작 시 폐허 상태다. 성의 규모가 커질 때마다 `마지막 성채 → 호수의 왕성 → 재건된 림가르드`로 외형과 지도 명칭이 변화한다.

### 4.2 발도르 군사 제국 — 25개

| ID | 한글명 | 영문명 | 지역 인상 | 특산·랜드마크 | 영지 관리 개성 |
|---|---|---|---|---|---|
| `castle_01` | 발도르 | Valdor | 수도 | 황금 옥좌 | 영향력 수입과 기사 계열 인재의 공훈 획득 증가 |
...
| `castle_11` | 그란델 | Grandel | 도시 | 대제국 은행 | 월간 금화 수입이 높지만 적 첩보의 표적이 되기 쉬움 |
...
| `castle_18` | 메르시아 | Mercia | 도시 | 남부 교역소 | 타 종족 인재 영입 불이익 감소 |
...
| `castle_22` | 펠리온 | Pellion | 성역 | 승리의 기념비 | 승전 후 질서와 영향력 상승 |
| `castle_23` | 델카스 | Delcas | 요새 | 남부 감시탑 | 인접 적 성의 전투단 정보를 한 단계 더 공개 |
| `castle_24` | 세라딘 | Seradin | 마도성 | 유리 천문대 | 적 마법과 날씨 관련 정보를 미리 확인 |
| `castle_25` | 벨로아 | Velloa | 도시 | 은빛 수로 | 영지 진흥의 질서 감소를 일부 상쇄 |
...
| `castle_48` | 던글레이드 | Dawnglade | 도시 | 새벽 초원 | 질서가 높을 때 방문 인재 증가 |
...
| `personnel` | 인재 관리 | Personnel | 소속 인물의 배치, 상태, 공훈과 충성 확인 |
| `facilities` | 시설 | Facilities | 성의 기본 및 특화 시설 확인과 선택 |
| `tavern` | 선술집 | Tavern | 의뢰, 소문과 방문 인재 확인 |
| `delegation` | 영지관 위임 | Delegation | 영지관, 운영 방침과 월간 예산 설정 |
| `monthly_report` | 월간 보고 | Monthly Report | 지난달의 영지 관리, 인물과 사건 결과 확인 |

### 5.2 성 관리 탭

| UI ID | 한글명 | 영문명 | 표시 내용 |
|---|---|---|---|
| `castle_summary` | 성 개요 | Castle Summary | 소유 진영, 규모, 영지관, 특성, 수입과 위험 |
| `castle_focus` | 중점 사업 | Monthly Project | 이번 달 사업과 담당 인물 지정 |
| `castle_roster` | 주둔 인재 | Stationed Characters | 성에 소속되거나 체류 중인 인물 |
| `castle_party` | 주둔 전투단 | Garrison | 방어 전투단과 원정 가능한 전투단 |
...
| `security` | 질서 | Security | 혼란 / 불안 / 안정 / 평온 |
...
| `project_security` | 민생 안정 | Secure the Realm | 범죄, 갈등과 첩자를 해결해 질서를 높인다 | 무력·매력 |
| `project_defense` | 성채 정비 | Fortify Castle | 성문, 장벽과 방어 장치를 보수한다 | 정치·무력 |
| `project_recruitment` | 인재 발굴 | Seek Talent | `[일반]`, 방랑 영웅과 영입 단서를 찾는다 | 매력 |
| `project_training` | 합동 훈련 | Joint Training | 주둔 인물과 전투단의 숙련을 높인다 | 통솔 |
...
| `policy_order` | 안정 | Stability | 질서와 점령지 회복 강화 |
| `policy_defense` | 수비 | Defense | 성 방어와 부상 회복 강화 |
| `policy_campaign` | 원정 | Campaign | 전투단 훈련, 이동과 공격 준비 강화 |
| `policy_talent` | 인재 | Talent | 탐색, 영입과 관계 활동 강화 |
...
| `delegate_frontier` | 전선 요새 | Frontier | 질서, 방어와 훈련을 우선 |
| `delegate_talent` | 인재 집결 | Talent | 인재 발굴, 영입과 회복을 우선 |
...
| `activity_recruit` | 영입 | Recruit | 발견한 인재를 진영에 영입 |
...
| `activity_reward` | 포상 | Reward | 공훈에 맞춰 금화 또는 장비 지급 |
...
| `report_characters` | 인재 소식 | Character News | 공훈, 관계, 충성, 부상과 영입 |
| `report_diplomacy` | 대륙 정세 | World Affairs | 진영 변화, 외교와 전쟁 |
...
| 영지관에게 위임 | Delegate | 영지관 방침으로 자동 처리 |
...
- 한 진영 안에서 첫 음절과 어미가 지나치게 반복되지 않게 한다.
- 성마다 반드시 하나의 특산·랜드마크와 하나의 영지 관리 개성을 가진다.
- 영지 관리 개성은 단순 수치 보너스보다 규칙 변화 또는 새로운 선택지를 우선한다.
...
2. 성별 지형 특성, 고유 특산과 진영별 개성을 데이터에 추가한다.
3. 번영, 기술, 질서, 방어와 성 규모를 추가한다.
4. 기본 중점 사업 8종과 UI 문자열을 추가한다.
5. 영지관 위임 방침과 인재 활동을 추가한다.
6. 성별 특산·랜드마크와 영지 관리 개성을 구현한다.
