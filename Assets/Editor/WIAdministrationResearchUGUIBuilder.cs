...
{
    public static class WIAdministrationResearchUGUIBuilder
    {
        private const string SourcePath = "Assets/Prefabs/Administration/WIAdministrationHeroesUGUI.prefab";
        private const string PrefabPath = "Assets/Prefabs/Administration/WIAdministrationResearchUGUI.prefab";

        // 연구 목록과 담당자를 페이지 카드로 선택하는 고정 UGUI 프리팹을 생성합니다.
        [MenuItem("WI/UI/Build Research UGUI")]
        public static void Build()
        {
            GameObject root = PrefabUtility.LoadPrefabContents(SourcePath);
            root.name = "WIAdministrationResearchUGUI";
            root.GetComponent<Canvas>().sortingOrder = 110;
            ...
            back.GetComponentInChildren<TMP_Text>().text = "연구 목록으로";

            WIAdministrationResearchUGUIController controller = root.AddComponent<WIAdministrationResearchUGUIController>();
            ...
            GameObject existing = GameObject.Find("WIAdministrationResearchUGUI");
            if (existing != null) Object.DestroyImmediate(existing);
            GameObject prefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPath);
            WIAdministrationUGUISceneUtility.InstantiateUnderSceneRoot(prefab);
            EditorSceneManager.MarkSceneDirty(SceneManager.GetActiveScene());
            EditorSceneManager.SaveOpenScenes();
            Debug.Log("전역 연구 UGUI를 생성하고 MainScene 정리 루트에 배치했습니다.");
        }
