...
{
    public sealed class WIAdministrationEventChoiceUGUIController : WIAdministrationUGUIPanelController
    {
        ...
        private void OnEnable()
        {
            ...
            {
                administrationController.UGUIEventChoiceRequested += Open;
            }
        }
        ...
        private void Open(WIAdministrationReportActionType type, string id)
        {
            actionType = type;
            actionId = id;
            if (administrationController.TryGetUGUIEventChoiceSnapshot(type, id, out WIAdministrationEventChoiceSnapshot snapshot) == false)
            {
                return;
            }
        ...
        {
            if (administrationController.ResolveUGUIEventChoice(actionType, actionId, choiceIndex) == false)
            {
                return;
            }
            modal.Hide();
